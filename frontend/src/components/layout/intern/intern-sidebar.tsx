...
const link2= [
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={cn(
        "w-64 bg-[#EBF5F4] border-r border-gray-200 pt-3 min-h-screen fixed md:relative z-50 transition-transform duration-300 ease-in-out",
        isOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
      )}>
        {/* Mobile Close Button */}
        <button
          onClick={() => setIsOpen(false)}
          className="md:hidden absolute top-0 right-0 p-1 text-black bg-white hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors"
